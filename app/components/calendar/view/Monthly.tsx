    ...
    rowValues.push(
      <TableRow key={index}>
        {dayTag.map((column, index) =>
          <TableCell key={index}>{cellDate.toString()}</TableCell>
        )}
      </TableRow>
    )
  }
...
*/
  ...
  for (let index = 0; index < weeks_in_month; index++) {
    rowValues.push(
      <TableRow key={index}>
        {dayTag.map((column, columnKey) =>
          <TableCell key={columnKey}>{cellDate.toString()} <br/> i: {index} - Key: {columnKey}</TableCell>
        )}
      </TableRow>
    )
  }
...
/*
  for (let index = 0; index < weeks_in_month; index++) {
    rowValues.push(
      <TableRow key={index}>
        {getcolumnValues().map((column, index) =>
          <TableCell key={index}>{cellDate.toString()}</TableCell>
        )}
      </TableRow>
    )
  ...
  function getcolumnValues() {
    for (let index = 0; index < dayTag.length; index++) {
      columnValues.push(
        <TableCell key={index}>{cellDate.toString()}</TableCell>
      )}
      cellDate = getNextDay(cellDate);
