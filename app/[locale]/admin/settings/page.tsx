...
import Email from "@/components/admin/Settings/Email";
import General from "@/components/admin/Settings/General";
import Finance from "@/components/admin/Settings/Finance";
import Pdf from "@/components/admin/Settings/Pdf";
              ...
              <div className="rounded-sm border border-divider bg-background shadow-default min-w-45">
                <ul className="flex flex-col">
                  {links.map((link, index) => (
                    <li key={index} className={`border-b border-divider px-3 py-4 capitalize hover:text-warning ${currentGroup === link ? "text-warning" : ""}`} onClick={() => { changeGroup(link) }}>{link}</li>
                  ))}
                </ul>
              </div>
            </div>
